...
from src.route_reconstruction import extract_local_vertices
...
def stage_numbers(line: LineRunState, pdf_path: str, context: dict[str, Any]) -> None:
    ...
    group = next((group for group in groups if group.line_id == line.line_id), None)
    if group is None:
        raise RuntimeError("Группа линии не найдена при извлечении чисел")
    line.stage = "numbers"
...
def _process_line(line: LineRunState, context: dict[str, Any]) -> None:
